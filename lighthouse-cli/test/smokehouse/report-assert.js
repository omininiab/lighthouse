...
function getEmptySmokeResult() {
  // Ok to cast since failures will be enumerated structurally, so the struture
  // is where failures should go.
  return /** @type {SmokeResult} */ ({
    lhr: {audits: {}},
    artifacts: {},
  });
}
...
function report(actual, expected, reportOptions) {
  ...
  // If no actual result, use a default empty one.
  actual = actual || getEmptySmokeResult();
